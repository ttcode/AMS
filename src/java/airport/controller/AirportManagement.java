 ...
 * @author Jean
 */
@WebServlet(name = "AirportManagement", urlPatterns = {"/airportManagement"})
public class AirportManagement extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        if (!AppCore.initPage(request, response))
            return ;
        request.setAttribute("headtitle", "Airport Management");
        
        EntityManager em = emf.createEntityManager();
        Query query = em.createNamedQuery("Airport.findAll");
        request.setAttribute("airportList", query.getResultList());
        
        request.getRequestDispatcher("jsp/airport/manageAirport.jsp").forward(request, response);
    }

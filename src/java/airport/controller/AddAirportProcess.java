 ...
 * @author Jean
 */
@WebServlet(name = "AddAirportProcess", urlPatterns = {"/addAirportProcess"})
public class AddAirportProcess extends HttpServlet
{
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException
    {
        ...
        {
            Logger.getLogger(AddAirportProcess.class.getName()).log(Level.SEVERE, null, e);
        }
    }
    ...
    private boolean formCheck(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        {
            EntityManager em = emf.createEntityManager();
            Query query = em.createNamedQuery("Airport.findByIcao");
            query.setParameter("icao", icao.toUpperCase());
            List<Airport> airport = (List<Airport>)query.getResultList();
            if (airport.isEmpty() == false)
                error += "- ICAO Code '" + icao.toUpperCase() + "' already used<br />";
        }
        if (error.isEmpty())

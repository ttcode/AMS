...
@WebServlet(name = "UpdateAircraftProcess", urlPatterns = {"/UpdateAircraftProcess"})
public class UpdateAircraftProcess extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException 
    {
        ...
        try
        {
            ...
            if (formCheck(request, response) == false) return ;
            ...
            if (request.getParameter("assignedFlight").isEmpty() == false)
            {
                query.setParameter("flightNumber", Integer.parseInt(request.getParameter("assignedFlight")));
                Flightplan flightplan = (Flightplan)query.getSingleResult();
                aircraft.setAssignedFlight(flightplan);
            }
            else
                aircraft.setAssignedFlight(null);
    ...
    private boolean formCheck(HttpServletRequest request, HttpServletResponse response) throws IOException
    {
        ...
        EntityManager em=emf.createEntityManager();
        Query query;
        if (aircraft_registration.equals(request.getParameter("old_aircraft_registration")) == false)
        {
            query=em.createNamedQuery("Aircraft.findByAircraftRegistration");
        ...
        if (request.getParameter("assignedFlight").isEmpty() == false)
        {
            ...
            query.setParameter("flightNumber", Integer.parseInt(request.getParameter("assignedFlight")));
            Flightplan flightplan = (Flightplan)query.getSingleResult();
            if (flightplan.getDepartureAirport().getIcao().equals(request.getParameter("position")) == false)
            {
                error += "- Aircraft position and assigned flight departure airport must match.<br />";
                result = false;
            }
        }
        if (result == false)
